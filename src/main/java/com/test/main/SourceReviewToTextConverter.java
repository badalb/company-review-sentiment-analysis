...
public class SourceReviewToTextConverter {
	...
	public static void convertToText() {
		...
		for (int nRowIndex = 1; nRowIndex < nRowCount; nRowIndex++) {
			// String aspectClass = sheet.getCellAt(0,
			// nRowIndex).getValue().toString().trim();
			// String aspectValue = sheet.getCellAt(1,
			// nRowIndex).getValue().toString().trim();
			...
			// List<String> aspectValues =
			// Arrays.asList(aspectValue.split("\\s*,\\s*"));
		...
		FileReaderWriter.write_file(aspectValuesList,
				"/Users/badalb/TravisCI/company-review-sentiment-analysis/src/main/resources/input/review.txt", false);
	}
